...
// 형변환에는 암시적 형변환과 명시적 형변화 2가지 형변환이 있다.
// 암시적 형변환(implicit type casting)
// 암시적 형변환은 더 작은 데이터타입의 값을 더 큰 데이터타입으로 바꿀 때
// 발생된다. 특별히 어떠한 명령어를 적어주지 않아도
// 자동으로 자바가 변환시켜준다.

// 명시적 형변환(explicit type casting)
// 명시적 형변환은 더 큰 데이터타입의 값으 더 작은 데이터탑으로 바꿀 때 
...
public class Ex03TypeCasting {
	...
	// 암시적 형변환
	// 더 작은 데이터타입의 값을 더 큰 데이터탑이에 옮겨 담거나
	// 정수 값을 실수로 바꿀 때 발생
	byte b =3;
	int number = b;
	...
	// 원래 오버플로우 언더프로우는 우리가 일반적인 방법으로는
	// 발생시킬 수 없지만
